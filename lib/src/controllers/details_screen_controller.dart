...
class DetailsPageController extends GetxController {
  ...
  Future<void> openFile(var url) async {
    final result = await OpenFileSafePlus.open(url);
  ...
  void isFavoriteLocalUpdate() {
    isFavoriteLocal.value = !isFavoriteLocal.value;
  }

  void itemCounterUpdate(value) {
    productImageNumber.value = value;
  }

  void isDeliveryUpdate() {
    isDescription.value = !isDescription.value;
  }

  void ratingUpdate(double value) {
    rating = value;
  }

  void isSpecificUpdate() {
  ...
  Future<ProductDetailsModel> getProductDetails(int proId) async {
    return await Repository().getProductDetails(proId).then((value) {
      ...
      // Sauvegarder dans l'historique local des produits consultés
      final data = value.data;
      if (data != null) {
        LocalDataHelper().addRecentlyViewed(
          id: proId,
          title: data.title ?? '',
        ...
        if (Get.isRegistered<HomeScreenController>()) {
          Get.find<HomeScreenController>().refreshLocalRecent();
        }
      }
  ...
  void updateColorId(String value) {
    colorId = value;
    update();
  }

  void updateColorValue(String value) {
    colorValue = value;
    update();
  }
