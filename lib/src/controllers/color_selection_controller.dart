...
class ColorSelectionController extends GetxController {
  ...
  void insertAttrNameToList({required String name, required int index}) {
    _selectedAttrNameList.removeAt(index);
    _selectedAttrNameList.insert(index, name);
    _generateAttrNames();
  }

  void insertAttrIdToList({required String id, required int index}) {
    _selectedAttrIdList.removeAt(index);
    _selectedAttrIdList.insert(index, id);
    _generateAttrIds();
  }

  void changeAttrSelection({required int attrIndex, required int value}) {
  ...
  void setAttrData(int attrSize, List<Attributes> data, List<ColorsData> colorList) {
    _attrSize = attrSize;
    _generateArray();
  }
